...
(define 
  ...
  (let 
    ...
    (if (open-code-primitive-binarizeable-operator? operator) 
     ...
     (execute-code 
       (make-instruction-sequence 
         ...
         ( ;
          (assign 
            ,target
            (op , (open-code-primitive-application-operator exp))
            (reg arg1)
            (reg arg2)
          )
         )
...
; (+ 1 2 3 4 5)を(+ (+ (+ (+ 1 2) 3) 4) 5)のようにする
(define 
  (compile-open-code-primitive-binarize exp)
  (let 
    ( ;
     (operator (open-code-primitive-application-operator exp))
     (operands (open-code-primitive-application-operands exp))
    )

    (if (< (length operands) 3) 
      exp
      (compile-open-code-primitive-binarize 
        (append 
          (list 
            operator
            (list operator (car operands) (cadr operands))
          )
          (cddr operands)
        )
      )
