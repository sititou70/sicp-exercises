...
(define 
  (make-execution-procedure inst labels machine pc flag stacks ops)
  (cond 
    ((eq? (car inst) 'assign)
...
(define 
  (lookup-stack stacks reg-name)
  (let 
    ( ;
     (val (assoc reg-name stacks))
    )

    (if val 
      (cadr val)
      (error 
        "Undefined register for stack: ASSEMBLE"
        reg-name
      )
    )
  )
