...
; accept-action-procedure!で、新しい手続きをその場で実行しないようにすると、回路の初期化が行われなくなる。
; 例えば、半加算器では、入力が0の場合、通常は各回路の初期値は次のようになる
; A = 0
; B = 0
; D = 0
; E = 1
; S = 0
; C = 0
; 一方で、新しい手続きをその場で実行しないようにすると、すべての初期値は0のままである。
; A = 0
; B = 0
; D = 0
; E = 0
; S = 0
; C = 0
; ここで、Aに1とセットすると、AがつながっているorゲートはD回路を1に更新するが、その先のandゲートはEが0であるためSを1に更新しない。
; もちろん、Aがつながっているand回路も、結果であるCが既に0であるため何の回路も更新しない。
; したがって、回路の初期化が不適切なため、シミュレーション結果も間違ったものになってしまう。
