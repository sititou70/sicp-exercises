...
(define 
  ...
  (define 
    (iter conjuncts prev-conjunct-frames)
    (if (empty-conjunction? conjuncts) 
      prev-conjunct-frames
      (iter 
        (rest-conjuncts conjuncts)
        (stream-flatmap 
          (lambda (frame1) 
            (stream-filter 
              (lambda (item) (not (eq? item 'failed)))
              (stream-map 
                (lambda (frame2) 
  ...
  (iter conjuncts (singleton-stream '()))
)
(define 
  (merge-frames frame1 frame2)
  (cond 
    ((eq? frame1 'failed) 'failed)
    ((eq? frame2 'failed) 'failed)
    ((null? frame1) frame2)
    ((null? frame2) frame1)
    (else
     (merge-frames 
       (add-bind-to-frame frame1 (first-binding frame2))
       (rest-bindings frame2)
     )
    )
  )
)
(define 
  (add-bind-to-frame frame new-bind)
  (if (eq? frame 'failed) 
    'failed

    (let 
      ( ;
       (new-var (binding-variable new-bind))
       (new-val (binding-value new-bind))
      )

      (let 
        ( ;
         (bind (binding-in-frame new-var frame))
        )

        (if (not bind) 
          (extend new-var new-val frame)
          (unify-match (binding-value bind) new-val frame)
        )
      )
    )
  )
