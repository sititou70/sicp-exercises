...
(define 
  ...
  (cond 
    ...
    ((try-catch? exp) (analyze-try-catch exp))
...
(define 
  (analyze-try-catch exp)
  (let 
    ( ;
     (try-proc (analyze (try-catch-try-exp exp)))
     (catch-proc (analyze (try-catch-catch-exp exp)))
    )

    (lambda (env succeed fail) 
      (define try-succeeded-flg false)

      (try-proc 
        env
        (lambda (try-value fail2) 
          (set! try-succeeded-flg true)
          (succeed try-value fail2)
        )
        (lambda () 
          (if try-succeeded-flg 
            (fail)
            (catch-proc 
              env
              succeed
