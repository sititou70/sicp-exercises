...
(define 
  ...
  ; varsとvalsからvarを削除した(vars . vals)を返す
  (define 
    (unbound-var vars vals)
    (cond 
      ((null? vars) (cons '() '()))
      ((eq? var (car vars))
       (unbound-var (cdr vars) (cdr vals))
      )
      (else
       (let 
         ( ;
          (res (unbound-var (cdr vars) (cdr vals)))
         )

         (cons 
           (cons (car vars) (car res))
  ...
  (define 
    ...
    (if (eq? env the-empty-environment) 
      ...
      (let 
        ...
        (if (= (length vars) (length (car res))) 
          (env-loop (enclosing-environment env))
          (set-first-frame! env res)
        )
      )
    )
  )
